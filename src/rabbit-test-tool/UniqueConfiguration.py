from command_args import get_optional_arg, get_mandatory_arg, get_mandatory_arg_validated, get_optional_arg_validated

class UniqueConfiguration:
    def __init__(self, args, suffix):
        self.suffix = suffix
        self.config_tag  = get_optional_arg(args, "--config-tag", self.suffix, "")
        ...
        self.volume = get_mandatory_arg(args, "--volume", self.suffix)
        self.filesystem = get_mandatory_arg(args, "--filesystem", self.suffix)
        self.tenancy = get_mandatory_arg_validated(args, "--tenancy", self.suffix, ["default","dedicated"])
        self.core_count = int(get_mandatory_arg(args, "--core-count", self.suffix))
        self.threads_per_core = int(get_mandatory_arg(args, "--threads-per-core", self.suffix))
