...
playlist_load_path=os.getenv('PLAYLIST_LOAD_PATH')
...
def get_playlist_entries(playlist_file):
    ...
    for playlist_entry in playlist_json['playlistEntries']:
        ...
        if playlist_load_path:
            entry.topology = os.path.join(playlist_load_path, entry.topology)
            entry.policy = os.path.join(playlist_load_path, entry.policy)

        if not os.path.exists(entry.topology):
            console_out("RUNNER", f"The topology file {entry.topology} does not exist")
            exit(1)

        if len(entry.policy) > 0 and not os.path.exists(entry.policy):
...
if playlist_load_path:
    common_conf.playlist_file = os.path.join(playlist_load_path, common_conf.playlist_file)
...
for config_number in range(1, 2):
    ...
    if unique_conf.policies_file != "none":
        if playlist_load_path:
            unique_conf.policies_file = os.path.join(playlist_load_path, unique_conf.policies_file)
        if not os.path.exists(unique_conf.policies_file):
...
for i in range(common_conf.repeat_count):
    ...
    for top_counter in range(0, len(playlist_entries)):
        ...
        for config_tag in configurations:
            ...
            unique_conf = unique_conf_list[0]
